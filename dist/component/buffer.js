...
import { clickHouseRowValidator, validateClickHouseTableName, validatePositiveInteger, } from "./validators";
...
export const claimBatch = internalMutation({
    ...
    handler: async (ctx, { table, limit }) => {
        ...
        const docs = table === undefined
            ...
            : await ctx.db
                .query("buffer")
                .withIndex("by_table_and_status", (q) => q.eq("table", table).eq("status", "pending"))
                .order("asc")
                .take(limit);
        const result = [];
...
export const retryFailed = mutation({
    ...
    handler: async (ctx, { table, limit }) => {
        ...
        const docs = table === undefined
            ...
            : await ctx.db
                .query("buffer")
                .withIndex("by_table_and_status", (q) => q.eq("table", table).eq("status", "failed"))
                .order("asc")
                .take(limit);
        for (const doc of docs) {
