...
import {
  ...
  quoteClickHouseTableName,
  validatePositiveInteger,
  validateReadOnlyClickHouseQuery,
  type ClickHouseRow,
} from "./validators";
...
export const flush = action({
  ...
  handler: async (ctx, { url, user, password, table, batchSize, maxRetries }) => {
    ...
    const byTable = new Map<
      string,
      { ids: (typeof rows)[number]["id"][]; data: ClickHouseRow[] }
    >();
    ...
    const succeeded: (typeof rows)[number]["id"][] = [];
    const failed: (typeof rows)[number]["id"][] = [];
    const errors: string[] = [];
    ...
    for (const [table, group] of byTable) {
      try {
        ...
        failed.push(...group.ids);
        errors.push(
          `Table ${table}: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
    }
    ...
    for (const [table, group] of byTable) {
      const tableSucceeded = group.ids.filter((id) =>
...
export const queryClickHouse = action({
  ...
  handler: async (_ctx, { url, user, password, query: sql }) => {
    ...
    const res = await fetch(
      `${url}/?query=${encodeURIComponent(fullQuery)}`,
      {
        method: "GET",
        headers: {
          "X-ClickHouse-User": user,
